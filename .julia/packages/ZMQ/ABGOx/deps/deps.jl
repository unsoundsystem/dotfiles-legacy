...
const libzmq = joinpath(dirname(@__FILE__), "usr/lib/libzmq.so")
function check_deps()
    global libzmq
    if !isfile(libzmq)
        error("$(libzmq) does not exist, Please re-run Pkg.build(\"ZMQ\"), and restart Julia.")
    end

    if Libdl.dlopen_e(libzmq) in (C_NULL, nothing)
        error("$(libzmq) cannot be opened, Please re-run Pkg.build(\"ZMQ\"), and restart Julia.")
    end
